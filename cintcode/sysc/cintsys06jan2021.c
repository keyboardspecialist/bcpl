/*
** This is the main program of the Cintcode interpreter written in C
** designed to run on most machines with Unix-like C libraries. It is
** designed to work for both 32 and 64 bit BCPL.
**
** (c) Copyright:  Martin Richards  21 Mar 2020
**
*/
...
defined. cinterp.o contains the function interpret and both fasterp.o and
...
04/11/2019
Tried to make the source of cintsys and cintpos identical controlled by whether
CINTSYSyes or CINTPOSyes is defined. This change is still being tested.
...
#ifdef CINTPOSyes
// Declare variable only used in cintpos.
...
// The function setraster is defined in rasterp.o, and in
// fasterp.o there is a dummy definition.
...
BCPLWORD Globupb=2000;  // MR 28/12/2019 used to be 1000
                        // also made similar a change in boot.b
BCPLWORD tallyupb, tallyvec, *tallyv;
...
// The interpreter cintasm is defined on fasterp.o or rasterp.o
...
// is compiled with CINTERPyes defined.
...
{ int rc;
...
#ifdef CINTPOSyes
    printf("\ncintpos64 08 Jan 2020\n\n");
#endif

#ifdef CINTSYSyes
    printf("\ncintsys64 08 Jan 2020\n\n");
#endif
  ...
  // Allocate boot's stack ang global vector
  ...
  W[rootnode+Rtn_icount] = 100000000; // Rough estimated of instructions/sec.
  ...
  */
  ...
  // This will call start in boot.b
...
{ register BCPLWORD i;
  ...
  { default: printf("\nBad sys number: %lld\n", LL W[p+3]);
    ...
    // case  Sys_tally:   // Now done in cinterp or rasterp
    ...
    { // Return the current build number
      ...
      res = bld_LinuxSDL2;
#elif forLinuxGL
      res = bld_LinuxSDLGL;   // forLinuxGL will be removed later
#elif forLinuxSDLGL
      res = bld_LinuxSDLGL;
#elif forLinuxSDL2GL
      res = bld_LinuxSDL2GL;
    ...
    case Sys_setraster:
      //printf("calling setraster %d %d\n", W[p+4], W[p+5]);
      res =  setraster(W[p+4], W[p+5]);
      // setrater is defined in rasterp with dummy
...
BCPLWORD doflt(BCPLWORD op, BCPLWORD a, BCPLWORD b, BCPLWORD c) {
  ...
  case fl_ldexp:  // Returns a x 2^b where a is floating
    ...
    case fl_64to32: // sys(Sys_flt, fl_64to32, a)
                    ...
                    // 64 bit Cintcode.
      { FLOAT32 a32 = (FLOAT32)N2F(a);  //(*((BCPLFLOAT*)(&a)));
      BCPLWORD b32 = a32;  //(BCPLWORD)(*((BCPLUINT32*)(&a32)));
      //printf("fl_64to32: a32=%f  b32=%d\n", a32, b32);
      return a32;  //(BCPLWORD)(*((BCPLUINT32*)(&a32)));
    }
  }
}
...
char instrtype(int f) {
  char *s = "0F0000000000RI10000000000000RIRI"
...
{ FILEPT fp;
  ...
  mem[rootnode + Rtn_sysp]    = sysp;
  mem[rootnode + Rtn_sysg]    = sysg;
  mem[rootnode + Rtn_sysst]   = sysst;
