...
and expressions, and the NEXT and FAIL commands. Labels are now
...
{ // This is only called after LET or AND.
  ...
  { CASE s_lparen: // This must be a fndef or rtdef
      { LET a = 0
        lex()
        // Check tha n is a single name not qualified by FLT.
	...
	// a is either zero or a name list
        ...
        IF token=s_eq DO
	{ lex()
          RESULTIS mk6(s_fndef, n, a, rexp(0), 0, ln)
        }
    ...
    { // This must be a function or routine defined using patterns.
      synerr("Pattern matching is not yet available")
...
{ // x -> [s_for, N, initval, lim, step, c, ln]
  ...
  // Set k, n to load the end limit if there is one
  ...
  ELSE { IF next<=0 DO blab := nextlab()
         // Only perfore a conditional jump if the TO expression was given.
	 IF k DO
         { out2(s_lp, s)
           out2(k, n)
