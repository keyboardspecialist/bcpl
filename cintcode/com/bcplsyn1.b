...
rdbpat   // Read a basic pattern
rdpat    // (n) Read a pattern of precedence n 
...
{ LET treesize = 0
  ...
  writef("*n%n bit BCPL (30 Oct 2021) with pattern matching, %n bit target*n",
...
{ LET a, op = 0, token
   ...
   { DEFAULT: synerr("Error in expression")
	...
	RESULTIS rbcom()
      ...
      CASE s_add:    RESULTIS rnexp(5) // No need to create a pos node
...
//writef("rdmatchitem: calling rdpat(0), token = %s*n", opname(token))  

  // Allow an empty pattern list between ':' and '=>' or BE.
  UNLESS token=s_yields | token=s_be DO patlist := rdpat(0) // ?????????
...
{ LET pat = 0   // pat will be the result
  ...
  { DEFAULT:
    ...
    CASE s_lparen:
      ...
      pat := rdpat(0)                // ( P0 )
      UNLESS token=s_rparen DO
        synerr("The ')' seems to be missing after '(' in a ( P ) construct")
      lex() // Skip over the ')'
      ENDCASE

    CASE s_true:     // Contants not allowed in ranges
    CASE s_false:
    CASE s_query:
      pat := rbexp()
      RESULTIS pat

    CASE s_number:   // Unsigned numbers
    CASE s_fnum:
    CASE s_name:     // Either a constant or a name
      pat := rbexp()
      GOTO chkrange

    CASE s_add:      // A signed number
    CASE s_fadd:
    CASE s_sub:
    CASE s_fsub:
    { // Only integer or floating point constants allowed here
      lex()
      UNLESS token=s_number | token=s_fnum ENDCASE
      TEST op=s_add
      THEN pat := rbexp()
      ELSE pat := mk2(op=s_fpos -> s_fpos,
                      op=s_sub  -> s_neg,
                      op=s_fsub -> s_fneg,
                      op, rbexp())
chkrange:      
      ...
      { LET t = 0
        lex()
        t := rdbpat()
        // The operands of range must be names or possibly signed
        // integers or floating point conatants.
        IF t SWITCHON h1!t INTO
        { DEFAULT:  ENDCASE
          CASE s_number:
          CASE s_fnum:
          CASE s_name:     // Either a constant or a name
            RESULTIS mk3(op, pat, t)
        }
        synerr("Bad operand for %n", opname(op))
      }
      RESULTIS pat
    }
  ...
  synerr("Operand of a range must be a name or number")
  RESULTIS 0
}

AND rdpat(n) = VALOF
{ LET pat = 0   // This will hold the result

  // First read a basic pattern
  SWITCHON token INTO
  { DEFAULT: synerr("First symbol of a pattern is wrong")
             RESULTIS 0
    ...
    { LET relop = rel2patrel(token)
      ...
      // If not an expression in parentheses the operand must be
      // one of the following;
      // a possibly signed number or fnum
      // NEXT EXIT BREAK LOOP or ENDCASE
      // TRUE or FALSE
      // query
      // a name
      pat := mk2(relop, rdbpat())
      ENDCASE
    }
    ...
    CASE s_sbra:
      lex()
      UNLESS token=s_sket DO pat := rdpat(0)
      UNLESS token=s_sket DO
        synerr("The ']' seems to be missing after '[' pattern-list")
      lex() // Skip over the ']'
      pat := mk2(s_patptr, pat)
      ENDCASE
    ...
    CASE s_flt:
      pat := rexp(9)  // Read FLT name
      ENDCASE

    // The only remaing possibility is a const
    CASE s_number:
    CASE s_fnum:
    ...
    CASE s_false:
    CASE s_query:
    CASE s_name:               // Either a manifest name or
                               // a local pattern variable
      pat := rdbpat()
      IF token=s_range | token=s_frange DO
      { LET op, ln = token, lineno
        pat := mk2(op, rdbpat())
      }
      ENDCASE  // pat is the first const
  }

  UNLESS pat DO synerr("Failed to read a pattern")

  // Now deal with the pattern operators

  { // Start of pattern opertor loop for
    // ','          for comma    precedence <1
    // '|'          for pator    precedence <2
    // juxtaposion  for patand   precedence <3
    LET op, ln = token, lineno
    ...
    { DEFAULT:
      ...
      CASE s_comma:
        IF n>=1 RESULTIS pat
        lex()
        pat := mk3(s_comma, pat, rdpat(0))  // Right associative operator
        LOOP
        
      CASE s_logor:
        IF n>=2 RESULTIS pat
        lex()
        pat := mk4(s_pator, pat, rdpat(2))
        LOOP
        
      // If token can start a pattern we must read the pattern and
      // form a patand node.
