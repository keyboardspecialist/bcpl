...
//    @h3!name, fop, opname(fop), prevt, prevk, prevk, ff, n, n)
    ...
    { // This has much in common with a SWITCHON command in that is
      ...
      // encountered.
      transnext(next) // Compile FNRN, a jump or nothing.
      RETURN
    }
...
//sawritef("IF exp BREAK/LOOP/ENDCASE optimised*n")
    ...
    CASE s_break:
      ...
      genjumpoptodest(s_jump, @breaklab)
      //IF breaklab=-1 DO { out1(s_rtrn); RETURN }
      //out2(s_jump, destlabel(op))
      RETURN
 
    CASE s_loop:
      context, comline := x, h2!x
      genjumpoptodest(s_jump, @looplab)
    ...
    CASE s_next:          // MR 28/08/2021
      context, comline := x, h2!x
      genjumpoptodest(s_jump, @nextlab)
    ...
    CASE s_repeatuntil:
    { LET l, bl, ll = genlab(), breaklab, looplab
      context, comline := x, h4!x
      breaklab, looplab := next, 0
      out2(s_lab, l)
      trans(h2!x, 0)
    ...
    CASE s_endcase:
      context, comline := x, h2!x
      genjumpoptodest(s_jump, @endcaselab)
      //IF endcaselab=-2 DO trnerr("ENDCASE out of context")
...
{ // Choose the label for a (possibly conditional) jump
  ...
  // BREAK, LOOP, ENDCASE, NEXT or LOOP.
